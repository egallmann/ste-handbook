...
The **system model** (in Part 4) is the conceptual whole carried by **Architecture IR**: not one diagram and not one document, but the **graph-shaped** description whose nodes and edges are the referents for **traceability**, **compilation** output, and **projection** generation. It is the structural counterpart to **intent**: **intent** says what is allowed and why; the system model says what the architecture **is** in terms of components, boundaries, and links—**as committed** through the pipeline.
