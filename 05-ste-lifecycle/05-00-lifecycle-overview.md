---
title: "Part 5: STE Lifecycle Overview"
status: structured
maturity: L1
diagrams: true
...
# Part 5: STE Lifecycle Overview

STE is a **lifecycle governance** system. It is not only a way to produce a first design. The same commitments and records must stay legible while **intent**, **architecture**, **embodiment**, and **evidence** move. This part walks the stages of that movement in order. It complements the control loop mechanics in Part 7 and the policy-oriented lifecycle chapters in Part 10.
...
- Artifact types: [Artifact layer overview](../03-artifact-layer/03-00-artifact-layer-overview.md).
- Canonical model: [Architecture IR overview](../04-architecture-ir/04-00-architecture-ir-overview.md).
- Loop mechanics: [Control loop overview](../07-control-loop/07-00-control-loop-overview.md).
- Policy and operations: [Lifecycle overview](../10-lifecycle-governance/10-00-lifecycle-overview.md).
